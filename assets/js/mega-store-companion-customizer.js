(function($) {
    ...
    // home slider
    wp.customize('mega_store_home_slider', function(value) {
        var oldObj;
        value.bind(function(to) {
            var repeater = JSON.parse(to);
            var focusIndex = 0;
            ...
            $.each(repeater, function(key, item) {
                ...
                html += '<div class="owl-slide">' +
                    ...
                    '<h2 class="mgs-slider-heading animation animated-item-1"> ' + item.heading + ' </h2>' +
                    '<div class="mgs-slider-desc animation animated-item-2">' + item.description + '</div>' +
                    '<a href="' + item.button1_url + '" class="btn animation animated-item-3 banner-link"> ' + item.button1_text + ' </a>' +
                    '<a href="' + item.button2_url + '" class="btn animation animated-item-3 banner-link"> ' + item.button2_text + ' </a>' +
                    '</div>' +
                    '</div>';
            });
    ...
    wp.customize('mega_store_home_services', function(value) {
        value.bind(function(to) {
            var repeater = JSON.parse(to);
            var html = '';
            $.each(repeater, function(key, item) {

                html += '<div class="col-md-3 col-sm-6 col-xs-6 service-item">' +
                    '<div class="service-item-inner">' +
                    '<div class="service-photo service-item-bga" style="background-image:url(' + item.image + ')"></div>' +
                    '<div class="service-inner-info">' +
                    '<div class="service-icon">' +
                    '<i class="fa ' + item.icon + '"></i>' +
                    '</div>' +
                    '<div class="service-info">' +
                    '<h3 class="service-title">' + item.heading + '</h3>' +
                    '<p class="service-desc">' + item.description + '</p>' +
                    '</div>' +
                    '<a class="btn service-read-more" href="' + item.button_url + '">' + item.button_text + '</a>' +
                    '</div>' +
                    '</div>' +
                    '</div>';
            });
    ...
    wp.customize('mega_store_home_features', function(value) {
        value.bind(function(to) {
            var repeater = JSON.parse(to);
            var html = '';
            $.each(repeater, function(key, item) {
    ...
    // testi slider
    wp.customize('mega_store_home_testimonials', function(value) {
        var oldObj;
        value.bind(function(to) {
            ...
            $.each(repeater, function(key, item) {
                ...
                html += '<div class="testimonial-item">' +
                    ...
                    '<h2 class="testimonial-name">' + item.heading + '</h2>' +
                    '</div>' +
    ...
    wp.customize('mega_store_home_brands', function(value) {
        var oldObj;
        value.bind(function(to) {
    ...
    wp.customize('mega_store_home_about_image', function(value) {
        value.bind(function(to) {
    ...
    wp.customize('mega_store_home_layout', function(value) {
        value.bind(function(to) {
            var items = to;
            var oldItem;
    ...
    var sections = [{
        ...
        {
            'heading': {
                'id': 'mega_store_home_product_tabs_heading',
                'selector': '.section-product-tabs .section-title',
            },
            'description': {
                'id': 'mega_store_home_product_tabs_desc',
    ...
    wp.customize('mega_store_body_font_size', function(value) {
        value.bind(function(to) {
            if (isNaN(to)) {
    ...
    wp.customize('mega_store_body_font_family', function(value) {
        value.bind(function(font_family) {
            ...
            if (google_fonts.indexOf(font_family) !== -1) {
                if($('#mega-store-fonts-style-css').length){
                    $('#mega-store-fonts-style-css').attr('href', insertParam2('family', font_family));
                }
                else{
                    $('head').append('<link id="mega-store-google-font-style-css" rel="stylesheet" type="text/css" href="'+insertParam2('family', font_family)+'">');
                }
